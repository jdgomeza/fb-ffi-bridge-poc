...
uint8_t* MyGame::CreatePlayerMovedBuffer(const Events::UUID& id, const Events::Point3D& ini, const Events::Point3D& end)
{
    flatbuffers::FlatBufferBuilder builder(1024);
    auto pm = Events::CreatePlayerMoved(builder, &id, &ini, &end);
    auto event = Events::CreatePlayerEvent(builder, get_ts(), Events::Event_PlayerMoved, pm.Union());
    builder.Finish(event);

    return builder.GetBufferPointer();
}

uint8_t* MyGame::CreateArrayPlayerMovedBuffer(std::vector<std::tuple<Events::UUID, Events::Point3D, Events::Point3D>> players)
{
    flatbuffers::FlatBufferBuilder builder(1024);
    return CreateArrayPlayerMovedBuffer(builder, players);
}

uint8_t* MyGame::CreateArrayPlayerMovedBuffer(flatbuffers::FlatBufferBuilder& builder, std::vector<std::tuple<Events::UUID, Events::Point3D, Events::Point3D>> players)
{
    std::vector<uint8_t> eventTypeVector;
    std::vector<flatbuffers::Offset<void>> eventVector;

    for (auto playerPos : players) {
        auto pm = Events::CreatePlayerMoved(builder, &std::get<0>(playerPos), &std::get<1>(playerPos), &std::get<2>(playerPos));
...
float MyGame::distance(float x1, float y1, float z1, float x2, float y2, float z2)
{
